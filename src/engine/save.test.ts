...
describe('save/load', () => {
  ...
  it('migrates a v1 save all the way to v8', () => {
    const storage = fakeStorage()
    const v1 = {
    ...
    expect(state!.teams[0]).toMatchObject({
      ...
      capacity: 25_000, ticketPrice: 15, fanMood: 50, managerHiredSeason: 0,
    })
    expect(typeof state!.teams[0].manager).toBe('string')
  ...
  it('migrates a v5 save to v8 with an empty news feed', () => {
    const storage = fakeStorage()
  ...
  it('migrates a v6 save to v8 with managers everywhere', () => {
    const storage = fakeStorage()
