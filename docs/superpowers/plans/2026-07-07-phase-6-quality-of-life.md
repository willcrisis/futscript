...
7. **Labeled controls** (`SquadScreen`, `TableScreen`, `FixturesScreen`): add a tiny shared local pattern — each select wraps in a `<label className="flex flex-col gap-0.5">` with a micro-label `<span className="text-[10px] font-semibold uppercase tracking-wider text-ink-faint">{t('squad.formation')}</span>` above the control (keys: `squad.formation`, `squad.tactic`, `squad.training`, `common.division`). The visible label replaces the aria-label (native label association covers it). The Friendlies checkbox label becomes `t('squad.friendlies')` ("Friendlies on free weeks" / "Amistosos nas semanas livres") with `title={t('squad.friendliesHint')}` ("Extra gate income on cup weeks you're not playing; normal injury risk"). Verify the controls row still wraps cleanly at 390px (labels above, not beside, prevent overflow).
